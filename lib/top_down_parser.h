...
private:
  ...
  void build_parsing_table() {
    ...
    vector<vector<string>> table(
        ...
        vector<string>(grammar.terminals.size() + 1, ""));
      ...
      for (auto it2 = 0; it2 < it->second.size(); ++it2) {
        ...
        for (auto it3 = first_temp.begin(); it3 != first_temp.end(); ++it3) {
          ...
          if ((*it3) != "#") {
            ...
            for (auto it4 = follow_temp.begin(); it4 != follow_temp.end();
                 ++it4) {
...
public:
  Parser(Grammar &grammar, map<string, set<string>> first_sets,
         map<string, set<string>> follow_sets) {
    ...
    while (!(st.top() == "$" && tokenized_input == "$")) {
      ...
      if (st_top == lookahead) {
        ...
        if (rule_to_be_applied == "") {
          cout << "ERROR: Invalid rule search [" << st_top << "," << lookahead
               << "]" << endl;
          // cout << "Current state of unconsumed input is " << tokenized_input
          // << endl;
          cout << "Rejected !" << endl;
          flag = false;
          break;
        ...
        st.pop();
        cout << "Production Rule applied: " << st.top() << "->"
             << rule_to_be_applied << "\n";
        if (rule_to_be_applied != "#") {

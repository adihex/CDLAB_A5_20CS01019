...
private:
  ...
  bool is_non_terminal(string str) {
    auto it =
        find(grammar.non_terminals.begin(), grammar.non_terminals.end(), str);
  ...
  set<string> follow(string symbol) {
    ...
    if (follow_set_being_computed.find(symbol) !=
            follow_set_being_computed.end() &&
        follow_set_being_computed[symbol]) {
        ...
        for (auto j = 0; j < occurred_positions.size(); ++j) {
          ...
          if (index < (rule.length() - 1)) {
            ...
            if (temp_first.find("#") != temp_first.end()) {
              follow_sets[symbol].erase("#");
              if (it->first != symbol) {
                set<string> temp_follow = follow(it->first);

                follow_sets[symbol].insert(temp_follow.begin(),
                                           temp_follow.end());
              }
            }
          }
  ...
  //     set<string> result;
  //     if (follow_set_being_computed.find(symbol) !=
  //     follow_set_being_computed.end() && follow_set_being_computed[symbol])
  //     {
  //         need_to_run_again = true;
  //         return;
  //     }
  ...
  //                     int temp_id = index + 1;
  //                     string temp_str = rule.substr(index + 1, rule.length()
  //                     - index); set<string> temp_first =
  //                     get_first_set_of_string(temp_str);
  ...
  //                             get_follow_set(it->first);

  //                             result.insert(temp_follow.begin(),
  //                             temp_follow.end());
  //                         }
  //                     }
  //                 }
...
public:
  ...
  Sets(Grammar grammar) { this->grammar = grammar; }

    ...
    if (it != isLeftRec.end() && it->second) {
      ...
      for (int j = 0; j < i->second.size(); ++j) {
        if (check_if_left_recursive(i->first, i->second[j])) {
          string rhs =
              i->second[j].substr(i->first.length(),
                                  i->second[j].length() - i->first.length()) +
              new_term;
        ...
        if (index - prev > 1) {
          ...
          if (max_lcp > 0) {
            ...
            for (int k = prev; k < index; k++) {
              string temp = i->second[k].substr(max_lcp, i->second[k].size() -
                                                             max_lcp + 1);
              if (temp == "") {
                temp = "#";
              }
...
void transform_to_single_char_grammar(string input_filename,
  ...
  int terms, non_terms, rule_count;
  string temp_term, temp_non_term, prod_rule, non_term, rules_rhs, temp,
      temp_start;
  string myText;
